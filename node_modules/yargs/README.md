...
Arguments without a correspondending flag show up in the `argv._` array.
...
Demand in context of commands. You can demand a minimum and a maximum number a user can have within your program, as well as provide correspondending error messages if either of the demands is not met.
```javascript
require('yargs')
  .command({
    command: 'configure <key> [value]',
...
parser.parse(bot.userText, function (err, argv, output) {
  if (output) bot.respondend(output)
})
```
...
correspondending description, will be reported as an error.
